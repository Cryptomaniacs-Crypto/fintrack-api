...
module FinanceTracker
  ...
  #
  # Encryption is symmetric (RbNaCl SimpleBox, keyed by TOKEN_KEY) so the
  # token is opaque to clients and unforgeable without the server key. This
  # is NOT a JWT — Google's id_token (verified by the SSO work) is the only
  # JWT in this project.
  class AuthToken
    extend Securable
    ...
    def self.load(token)
      ...
      instance.instance_variable_set(:@scope, AuthScope.new(contents['scope']))
    ...
    # The AuthScope carried by the token. On a loaded token this is rebuilt
    # from the scope string persisted at mint time.
    def scope
      raise ExpiredTokenError if expired?
