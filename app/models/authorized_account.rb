...
module FinanceTracker
  ...
  #
  #   2. Reconstruction (HttpRequest): wrap the decrypted token payload (a hash
  #      that already carries the id under 'attributes') plus the token's scope,
  #      exposing `.account` (a hash) and `.scope` for controllers and policies.
  class AuthorizedAccount
    ...
    # token small. The rich envelope still rides in the plaintext `to_h`
    ...
    # Handles both the FinanceTracker display envelope ({data: {attributes}})
    # and a bare {attributes} payload.
    def account_attributes
      return {} unless @account.is_a?(Hash)

      @account.dig('data', 'attributes') || @account['attributes'] || @account
    end
  end
end
